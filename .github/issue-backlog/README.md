...
> **Snapshot at generation time (June 9, 2026) — the live tracker is canonical.** Known supersessions: priorities, labels, and milestone membership changed in the June 11 audits; the `2.0.0-alpha.N` versioning in 001/002 was replaced by the odd-minor scheme (pre-release = `1.99.x` + `--pre-release`, GA = `2.0.0` — see issue #243). Do not regenerate issues from these files.
